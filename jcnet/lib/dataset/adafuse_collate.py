...
from multiviews import cameras_cuda_col as cameras_utils
...
def adafuse_collate(data):
    ...
    for bv in range(nitems):  # iterate through all samples in a batch
        ...
        action.append(m['action'])
        subaction.append(m['subaction'])
        subject.append(m['subject'])
        image_id.append(m['image_id'])
    ...
    metas_collate['action'] = torch.as_tensor(np.array(action), dtype=torch.float32)
    metas_collate['subaction'] = torch.as_tensor(np.array(subaction), dtype=torch.float32)
    metas_collate['subject'] = torch.as_tensor(np.array(subject), dtype=torch.float32)
    metas_collate['image_id'] = torch.as_tensor(np.array(image_id), dtype=torch.float32)
