...
class SoftArgmax2D(torch.nn.Module):
    ...
    def forward(self, x, out_smax=False, hardmax=False):
        ...
        maxv, argmax = torch.max(x.view(batch_size * channels, -1), dim=1)  # argmax over the W and H dimensions
        #   maxv每列最大值,argmax每列最大值的索引
        #   maxv: (batch_size * channels, 1), argmax: (batch_size * channels, 1)
        argmax_x, argmax_y = torch.remainder(argmax, width).float(), torch.floor(torch.div(argmax.float(), float(width)))
        
        #   torch.remainder()取余数，torch.floor()取整数部分
        windows = _make_radial_window(width, height, argmax_x, argmax_y, self.window_fn, self.window_width)
        #   _make_radial_window()生成窗口
        ...
        # xys_max是经过softmax和窗口函数处理后的概率分布的坐标
        ...
        # x_coords是经过softmax和窗口函数处理后的概率分布的x����标
        # x_coords: (batch_size, channels)
        ...
        xs = torch.unsqueeze(x_coords, 2) #第三维度加1
        # xs: (batch_size, channels, 1)
