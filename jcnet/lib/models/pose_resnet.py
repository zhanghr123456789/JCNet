...
class PoseResNet(nn.Module):
    ...
    def __init__(self, block, layers, cfg, **kwargs):
        ...
        self.deconv_layers = self._make_deconv_layer(
            ...
            cfg.POSE_RESNET.NUM_DECONV_KERNELS,            #1
        )
    ...
    def forward(self, x):
        ...
        '''
        # 求2D关节的遮挡信息
        # x的输出形式是[batch_size, 20, 64, 64]
        x = x.view(x.size(0), x.size(1), -1)
        # x的输出形式是[batch_size, 20, 4096]

        x = x.mean(2)
        # x的输出形式是[batch_size, 20]
        # x.mean(2)是求每个batch的平均值
        x = x.view(x.size(0), -1)
        # x的输出形式是[batch_size, 20]
        x = torch.sigmoid(x)
        # x的输出形式是[batch_size, 20]
        ...
        return x, x_final_feature       #x为20个通道的热图结果，x_final_feature为256通道数的特征图
